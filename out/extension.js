...
function activate(context) {
    let disposable = vscode.commands.registerCommand('reidViewer.viewImages', async (resourceUri) => {
        let dirPath;
        ...
        const panel = vscode.window.createWebviewPanel('reidViewer', 'ReID Viewer', vscode.ViewColumn.One, {
            enableScripts: true,
...
function findImagesDirectory(dirPath) {
    try {
...
function loadSimilarityDataWithMapping(csvPath, expectedImageCount) {
    try {
...
function getWebviewContent(imageFiles, dirPath, similarityData, useIndexMapping, webview, extensionUri) {
...
<body>
    ...
    <script>
        ...
        ${Array.from(similarityData.entries()).map(([key, valueMap]) => `similarityDataObj["${key}"] = ${JSON.stringify(Object.fromEntries(valueMap))};`).join('\n        ')}
